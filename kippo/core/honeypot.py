...
import random, pickle, time, shlex, anydbm
...
class HoneyPotCommand(object):
    ...
    def start(self):
        kippo.core.constants.current_command = self.honeypot.cmd
    ...
    def getAction(self):
        print "getAction start", kippo.core.constants.current_command
        global RL
        RL.go()
        while True:
            if kippo.core.constants.rl_action is not None:
                print "********************", kippo.core.constants.rl_params
                return kippo.core.constants.rl_action[0]
...
class HoneyPotEnvironment(object):
    def __init__(self):
        ...
        for c in kippo.commands.__all__:
            module = __import__('kippo.commands.%s' % c, globals(), locals(), ['commands'])
...
class HoneypotPasswordChecker:
    ...
    credentialInterfaces = (credentials.IUsernamePassword, credentials.IPluggableAuthenticationModules)
